...
from bot.hf.fic import fdbckbt_replies_handler
from bot.hf.flifi import fdbckbt_chats_handler
from bot.hf.stuf import get_command_msg_n_reason
...
@Client.on_message(
    ...
    fdbckbt_chats_handler([AUTH_CHANNEL]) &
    fdbckbt_replies_handler
)
async def un_ban_command(client: Client, message: Message):
    ...
    if not user_id:
        return
    _, unban_reason = get_command_msg_n_reason(message.text)
